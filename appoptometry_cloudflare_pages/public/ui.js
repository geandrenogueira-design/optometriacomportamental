...
// ---------------- Panel Markup Guards ----------------
  ...
  function initPanel(){
    ...
    el('btn_panel_copy')?.addEventListener('click', ()=>{
      const p = getSelectedPatient();
      if(!p){ showToast('Selecione um paciente primeiro.', false); return; }
      const box = el('panel_integrado');
      const txt = box ? box.innerText : '';
      copyToClipboard(txt)
        .then(function(){ showToast('Painel copiado para a área de transferência.'); })
        .catch(function(){ showToast('Não consegui copiar automaticamente. Selecione e copie manualmente.', false); });
});
  }
  ...
  function setSyncStatus(txt, ok){
    ...
    return fetch(SYNC_API_URL, { headers: { 'X-Sync-Key': key }})
      .then(res=>{
        if(res.status === 404){
      ...
      .then(data=>{
        ...
        else if(typeof renderPatientList === 'function') renderPatientList();
      ...
      function syncPush(){
    const key = getSyncKey();
    if(!key){ setSyncStatus('Informe a chave', false); return Promise.resolve(false); }
    ...
    .catch(e=>{
      ...
      setSyncStatus('Falhou', false);
      try{ showToast('Falha ao enviar: '+(e && e.message ? e.message : e), false); }catch(_){}
      return false;
    });
  }
